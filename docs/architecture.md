...
当前专用适配器覆盖腾讯、网易、华为、大疆、滴滴、联想、Amazon 中国和 Apple 中国。它们统一实现 `CareerAdapter` 协议，但可以分别处理 JSON API、Moka 初始化数据、Avature HTML 和服务端 hydration JSON。专用适配器返回空集或失败时，采集器会记录真实状态并尝试通用 HTML 降级；只有解析到岗位的来源才标记为 `healthy`。
...
来源健康度按公司做快照替换，避免官网改版后旧的 `healthy` 记录长期残留。状态语义为：

- `healthy`：本次确实解析到公开岗位；
- `empty`：入口可访问，但没有可解析岗位；
- `restricted`：页面要求登录或验证码；
- `blocked_by_robots`：robots 明确禁止自动访问；
- `error`：网络、协议或页面结构异常。
